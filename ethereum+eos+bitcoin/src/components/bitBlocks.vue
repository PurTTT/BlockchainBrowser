    ...
    <el-container>
      ...
      <div class="table-header">
        <div class="table-row">
          ...
          <div class="table-column">区块字节数</div>
          <div class="table-column">区块哈希</div>
          <div class="table-column">区块索引</div>
      ...
      <div v-if="loading || blocks.length" class="table-body" :class="`${loading ? 'table-body-loading' : ''}`">
        <div class="table-row" v-for="(block,i) in blocks" :key="`block-${i}`">
          ...
          <div class="table-column" v-text="block.n_tx"></div>
...
//存blocknum
      var blockNumber = 0;
      var blocks = [];
      var block ={'time':0,'n_tx':0};
      let txs = [];

//axios的定义
      const axios = require('axios');
//定义变量
      var netUrl = "http://127.0.0.1:5000/bitInfo";
...
//axios请求结束
        ...
        // block.n_tx = block.n_tx;
        blocks.push(block);
        txs = txs.concat(block.tx);
      }
//for end
        console.log(txs);
        console.log('blocks');
        ...
        for (let i = 1; i < 15; i++) {
          ...
          Object.keys(result.blocks).map(key => {
            if(result.blocks[key].number === handledTx.blockNumber){
              handledTx.time = result.blocks[key].time;
            }
          })
